...
// schema.prisma): the Films shelf on a show's page, the "Part of …" chips on
// a film's page, and the list the owner's picker chooses from. Server-only,
 ...
 *  as missing, and concerts are never linkable (the picker is on the film
 *  page, but this is the boundary). Age-filtered exactly as getConcerts is —
...
/** The shows a film is linked to, for its "Part of …" chips. Age-filtered
...
/** Every show the owner may link a film to, alphabetically — the whole
 *  library, because there are few shows and a picker that hides some of them
 *  would be a puzzle. Age-filtered too: the app owner is normally
 *  unrestricted, but nothing guarantees it. */
export async function getLinkableShows(limit: AgeLimit): Promise<LinkedShow[]> {
