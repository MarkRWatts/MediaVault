...
function reportProgress(versionId: number, positionSecs: number, durationSecs: number, isNewPlay: boolean) {
  if (!Number.isFinite(durationSecs) || durationSecs <= 0) return;
  fetch(`/api/video/${versionId}/progress`, {
    method: "POST",
...
export default function VideoPlayer({ versionId, title, onClose }: { versionId: number; title: string; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    async function check() {
      const [statusRes, progressRes] = await Promise.all([
        fetch(`/api/video/${versionId}/status`, { cache: "no-store" }),
        // Best-effort: if this fails for any reason we just don't resume —
        // not worth blocking playback over.
        fetch(`/api/video/${versionId}/progress`, { cache: "no-store" }).catch(() => null),
      ]);
      if (cancelled) return;
  ...
  const flushProgress = useCallback(() => {
    ...
    reportProgress(versionId, v.currentTime, v.duration, false);
  }, [versionId]);
                ...
                onPlaying={() => {
                  ...
                  if (!hasReportedStartRef.current) {
                    ...
                    if (v && Number.isFinite(v.duration) && v.duration > 0) {
                      lastReportedPosRef.current = v.currentTime;
                      reportProgress(versionId, v.currentTime, v.duration, true);
                    }
                  }
                }}
