...
//
// The allow-list check AND the send run detached from the caller: awaiting
// them made an allowed address take a Resend round trip longer to answer
// than a stranger's, which is a measurable "is this email known?" oracle.
// Now every request returns as soon as the plugin has stored the code, and
// a send failure is logged rather than reported (the sign-in page already
// says "check your email" either way, and offers a resend).
