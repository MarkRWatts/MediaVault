...
| 4 | **Post-OTP nudge** — **done**. `verifyOTP` sets `mv-passkey-nudge` (httpOnly, 24h — the same span as the plugin's fresh-session window, so the strip never outlives the ability to act on it; not set on the Jellyfin-SSO branch, which redirects off-site). The library page reads it and renders `PasskeyNudge`, which shows itself only on a passkey-capable device (same `usePasskeySupport` hook) and until dismissed; dismissal is `localStorage` read through `useSyncExternalStore`, with "Add a passkey" (→ `/account#passkeys`) counting as a dismissal too. Deliberately no "already has a passkey" check: the server can't know whether *this* device holds one, and a synced iCloud passkey on the phone says nothing about the Windows PC they just signed into. Verified in the same e2e run (six more checks): absent without the cookie, shown with it, hidden by "Not now", still hidden after reload, shown again once storage is cleared, link lands on `/account#passkeys` and dismisses | 0.5 day |
...
| 6 | **Verification + docs + deploy** — harness and docs **done**, manual pass outstanding. `scripts/e2e-passkey.ts` (`npx tsx scripts/e2e-passkey.ts`, after a one-time `npx playwright install chromium`) is self-contained: scratch SQLite via `prisma migrate deploy`, sessions seeded straight into the DB (the cookie is `encodeURIComponent(token + "." + base64(HMAC-SHA256(secret, token)))`, so no Resend involved), a `next dev` it starts in its own process group and tears down, Chromium + a CDP virtual authenticator. 31 checks across register / rename / sign-in / revoked-member refusal / remove / not-found / anonymous-401 / stale-session / nudge; scratch dir kept on failure with the server log tail printed. `playwright` is a devDependency (no postinstall, so `npm ci` never downloads a browser). Deliberately not a vitest test: CI has no browser. README "Tests" documents it. **Still to do by hand:** a real iPhone + Safari-on-Mac pass after deploy — iCloud Keychain sync between them is the headline feature and the one thing a virtual authenticator can't exercise. | 0.5 day |
