...
| Phase | Work | Est. |
|---|---|---|
| 1 | **Plumbing.** `npm i @better-auth/passkey@1.7.2`. `src/lib/auth.ts`: add `passkey({ rpName: "MediaVault", origin: process.env.BETTER_AUTH_URL, authenticatorSelection: { residentKey: "required", userVerification: "preferred" }, registration: { afterVerification: → logAudit("passkey.add") } })` to `plugins` (before `nextCookies()`, which must stay last). `src/lib/auth-client.ts`: `createAuthClient({ plugins: [passkeyClient()] })` from `@better-auth/passkey/client`. Schema + `migrate dev --name add_passkeys`. Tests (real temp SQLite, `src/lib/test-temp-db.ts` pattern): (a) `Passkey` rows cascade on `user.delete`; (b) a passkey holder whose email no longer passes `isAllowedEmail` is refused a session — mock at the hook boundary the way `require-member.test.ts` mocks `getSession`. `npx prisma generate`, `next typegen`, lint, typecheck, `npm test` all green before anything is visible. | 0.5 day |
| 2 | **`/account` → "Passkeys" section** (between identity and Preferences). Server component reads `prisma.passkey.findMany({ where: { userId }, orderBy: { createdAt: "asc" } })` — consistent with the page's other direct Prisma reads, no client-side atom — and passes rows to a `PasskeyManager` client component. Per row: name, "added <date>", a small "synced" / "this device only" badge from `backedUp`, inline rename (`updatePasskeyName` server action → `auth.api.updatePasskey({ body: { id, name }, headers })` + audit), remove with a confirm (`removePasskey` action → `auth.api.deletePasskey` + audit). "Add a passkey" is client-side by necessity: prompt for a name pre-filled from a tiny UA sniff (`iPhone` / `iPad` / `Mac` / `Windows` / `Android` / `This device` — a pure helper in `src/lib/passkey-label.ts` with unit tests), then `authClient.passkey.addPasskey({ name })`, then `router.refresh()`. **Fresh-session UX:** catch the plugin's fresh-session rejection and render "To add a passkey, sign out and back in first" with the sign-out button inline, instead of a raw error. Restyle to the page's existing tokens (`border-border`, `bg-bg-elevated`, `text-accent`), mirroring `AdultAccessToggle`'s `useActionState` shape. | 1 day |
| 3 | **`/signin` → passkey sign-in.** A `PasskeySignInButton` client component under the email form, rendered only when `oauthQuery === undefined` (see Auth & gating) and only when `window.PublicKeyCredential` exists (SSR-safe: render nothing until mounted). Click → `authClient.signIn.passkey()` → on success `router.push(callbackURL)` + `router.refresh()` (the `SignOutButton` pattern, so `proxy.ts` and every `getSession()` see the new cookie immediately). **Conditional UI / autofill:** on mount, fire `authClient.signIn.passkey({ autoFill: true })` and give the email input `autoComplete="username webauthn"` — iOS/Android/Chrome then offer the passkey inside the email field's own autofill sheet, which is the best-case one-tap flow; `@simplewebauthn/browser` aborts the pending autofill ceremony automatically when the button's modal ceremony starts. Error mapping per Auth & gating. `callbackURL` reuses `safeCallbackURL` — the button never redirects anywhere the page didn't already validate. | 0.5–1 day |
